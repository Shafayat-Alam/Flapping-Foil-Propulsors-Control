...
Subscribes: joint_cmd (Float32MultiArray) - from controller.py
            /joint_states (JointState) - bridged from gz (sim servo feedback)
Publishes:  joint_feedback (Float32MultiArray) - merged real + sim feedback to controller.py
            <cmd_topic_prefix>/<joint>/cmd_pos (std_msgs/Float64) - per-joint
              position commands to the gz JointPositionController (via ros_gz_bridge)

Sim actuation is position-based (gz JointPositionController). Velocity-mode
commands are not driven in sim (the gz position controllers expect positions);
the gazebo launch uses operating_mode 'position'.
"""
...
class GazeboDynamixelInterface(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('cmd_topic_prefix', '/prototype')

        self.joint_names = self.get_parameter('joint_names').value
        self.servo_ids = self.get_parameter('servo_ids').value
        self.port_name = self.get_parameter('port').value
        self.baudrate = self.get_parameter('baudrate').value
        self.cmd_topic_prefix = self.get_parameter('cmd_topic_prefix').value
        ...
        # Publishers — one Float64 position command per joint (bridged to gz).
        # CAD joint names may contain spaces ("Revolute 29"); topic names cannot,
        # so spaces are replaced with '_' (must match <topic> in robot_gz.xacro
        # and the ROS_TO_GZ entries in config/ros_gz_bridge.yaml).
        self.joint_cmd_pubs = {
            jn: self.create_publisher(
                Float64, f"{self.cmd_topic_prefix}/{jn.replace(' ', '_')}/cmd_pos", 10)
    ...
    def _cmd_callback(self, msg: Float32MultiArray):
        ...
        # Send to GAZEBO: per-joint position command (gz JointPositionController)
        for sid, mode, value in zip(ids, modes, values):
            joint_name = self.id_to_joint.get(sid)
            if joint_name is None:
                continue
            if mode == 3:  # position
                cmd = Float64()
                cmd.data = float(value)
                self.joint_cmd_pubs[joint_name].publish(cmd)
            elif not self._warned_velocity:
                self.get_logger().warn(
                    "Velocity-mode commands are not driven in Gazebo "
                    "(gz position controllers expect positions). Use "
                    "operating_mode 'position' for sim.")
