...
Subscribes: /camera/image_raw (sensor_msgs/Image) - from Gazebo (fallback)
            robot_cmd (std_msgs/String) - command start
            telemetry (std_msgs/Float32MultiArray) - command completion
"""
...
class GazeboStellarHDInterface(Node):
    ...
    def __init__(self):
        ...
        # Subscribers for command synchronization
    ...
    def _cmd_callback(self, msg: String):
        """Start recording on new command."""
        try:
            raw = msg.data.lower()
    ...
    def _telemetry_callback(self, msg: Float32MultiArray):
        """Stop recording on command completion."""
        if len(msg.data) < 1:
            return
    ...
    def _start_recording(self, cmd_id: float):
        ...
        with self.recording_lock:
            ...
            filename = f"cmd_{int(cmd_id)}_{timestamp}_{source}.mp4"
